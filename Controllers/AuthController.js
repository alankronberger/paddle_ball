...
class AuthController {
    ...
    async changePassword (username, password, newPassword) {
        const result = await this.UserModel.getPasswordHash(username);
        const verified = await this.verifyPassword(result.passwordHash, password);
        if(verified){
            const newHash = await this.hashPassword(newPassword);
            console.log(newHash);
            await this.UserModel.updatePasswordHash(username, newHash);
            return true;
        }
        else{
            return false;
    ...
    async updateUsername (username, newUsername){
        await this.UserModel.updateUsername(username, newUsername);
    }
